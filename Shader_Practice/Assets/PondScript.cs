...
public class PondScript : MonoBehaviour
{
    ...
    {   
        ...
        if(hookCollision){
            ...
            Compute.SetInt("xCoord",xCoord);
            Compute.SetInt("yCoord",yCoord);
        }
        else{
            Compute.SetFloat("dens",0);
            Compute.SetFloat("vX",0);
    ...
    void OnTriggerEnter(Collider collision) {
        Debug.Log("Triggered");
        //hookCollision = true;
        if(collision.gameObject.name == "fishhook(Clone)"){
            
            velocityX = -collision.gameObject.GetComponent<Rigidbody>().velocity.x*hookSpeedScaler;
            velocityY = -collision.gameObject.GetComponent<Rigidbody>().velocity.z*hookSpeedScaler;
            Vector3 relativePosition = transform.position - collision.gameObject.transform.position;
            //Debug.Log(relativePosition);
            relativePosition = new Vector3((1+(100*relativePosition.x/transform.localScale.x))/2,relativePosition.y,(1+(100*relativePosition.z/transform.localScale.y))/2);
            //Debug.Log(relativePosition);

            xCoord = (int)(relativePosition.x*N);
            yCoord = (int)(relativePosition.z*N);
           
            Debug.Log((int)(relativePosition.x*N));
            Debug.Log((int)(relativePosition.z*N));
            //Debug.Log(collision.gameObject.GetComponent<Rigidbody>().velocity.z*hookSpeedScaler);
            hookCollision = true;
        }
    }
